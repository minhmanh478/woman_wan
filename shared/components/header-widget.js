...
(function () {
  ...
  function setupHeaderInteractions(container, config, prefix) {
    ...
    function renderCart() {
      ...
      if (shippingEl) shippingEl.textContent = isFreeShipping ? "Miễn phí" : formatVND(shipping);
      ...
      if (progressText && progressBar) {
        if (subtotal >= FREE_SHIPPING_THRESHOLD) {
          progressText.innerHTML = "Chúc mừng! Bạn đã được <strong>Miễn phí giao hàng</strong>";
          progressBar.style.width = "100%";
          progressBar.style.backgroundColor = "#0050d8";
        } else {
    ...
    async function renderSavedItems() {
      ...
      if (wishlist.length === 0) {
        listEl.innerHTML = `
          <div class="empty-cart-state" style="text-align:center;padding:40px 16px;color:#767676;">
            <p style="font-size:15px;margin-bottom:8px;">Danh sách yêu thích trống.</p>
            <p style="font-size:13px;color:#999;">Hãy lưu lại những sản phẩm bạn yêu thích để xem lại sau!</p>
          </div>
        `;
        return;
      }
